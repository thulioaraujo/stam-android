...
namespace visual_odometry {
	...
	std::pair<int, int> Memory::addCorrespondence(int key_frame_id, const cv::Point2d& p2D, const cv::Point3d& p3D){
	...
	std::pair<int, int> Memory::addCorrespondence(int key_frame_id, const cv::Point2f& p2D, const cv::Point3f& p3D){
		return addCorrespondence(key_frame_id, cv::Point2d(p2D.x, p2D.y), cv::Point3d(p3D.x, p3D.y, p3D.z));
	}

	std::pair<int, int> Memory::addCorrespondence(int key_frame_id, const cv::Point2d& p2D, int point3d_id) {
	...
	std::pair<int, int> Memory::addCorrespondence(int key_frame_id, const cv::Point2f& p2D, int point3d_id) {
	...
	void Memory::optimise() {
		...
		if (!bal_problem.LoadCeresSBAParams(map_, points2D_, Rs_, Ts_, projections_,
				visibility_, cam_matrix_list, dist_coeff_list)){
			LOGE("Error to load ceres sbaParams!");
		}
		...
		for (int i = 0; i < bal_problem.num_observations(); ++i) {
			...
			problem.AddResidualBlock(cost_function,
					NULL /* squared loss */,
					bal_problem.mutable_camera_for_observation(it->first),
					bal_problem.mutable_point_for_observation(it->second.first));
			it++;
		}
		...
		std::map<int,int> kf_map_ = bal_problem.kf_map();
		int img_index;
		int kf_index = 0;
		for(auto it = kf_map_.begin(); it != kf_map_.end(); it++) {
			...
			for (int j = 0; j < 3; j++) {
				Rs_[img_index].at<double>(j,0) = bal_problem.mutable_cameras()[kf_index*9+j];
				Ts_[img_index].at<double>(j,0) = bal_problem.mutable_cameras()[kf_index*9+j+3];
			}
		}

		int num_cameras_ = Rs_.size();
		int i = 0;
		for (auto it = map_.begin(); it != map_.end(); it++, i=i+3) {
			it->second.x = bal_problem.mutable_points()[i];
			it->second.y = bal_problem.mutable_points()[i+1];
			it->second.z = bal_problem.mutable_points()[i+2];
		}

		LOGE("Preparing bundle adjustment data!\n");
	}
#endif
...
#ifdef WINDOWS_VS
	void Frame::detectAndDescribe(const cv::Mat &roi) {
		...
		detector->detect(image, kpts, roi);
		for (auto it = kpts.begin(); it != kpts.end(); it++)
		{
			//if (!roi.empty() && roi.at<uchar>((*it).pt)==255)
			//{
