...
namespace visual_odometry {
	...
	public:
		...
		Frame(const cv::Mat &image, const cv::Mat &depth) : image(image), depth(depth), id_(next_id_s_++) {}
...
#ifdef WINDOWS_VS
		void detectAndDescribe(const cv::Mat &roi = cv::Mat());
#elif ANDROID
		void detectAndDescribe(const cv::Mat roi = cv::Mat());
#endif
	};
	...
	public:
		...
		std::pair<int, int> addCorrespondence(int key_frame_id, const cv::Point2d& p2D, const cv::Point3d& p3D);
		std::pair<int, int> addCorrespondence(int key_frame_id, const cv::Point2f& p2D, const cv::Point3f& p3D);

		// Add correspondence to memory belonging to a specific keyframe identified by its keyframe_id
		// A correspondence is a 3D point id and its corresponding projection on the image plane
		// return: (p3d_id, p2d_id)
		std::pair<int, int> addCorrespondence(int key_frame_id, const cv::Point2d& p2D, int point3d_id);
		std::pair<int, int> addCorrespondence(int key_frame_id, const cv::Point2f& p2D, int point3d_id);
